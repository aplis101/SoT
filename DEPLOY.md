...
2. **SQL Editor** ← نفّذ بالترتيب. **الترتيب ملزم** — كل ملف يفترض ما قبله:
   ...
   | ١٠ | `hadith-sot/22-governance.sql` | الرتب الثلاث · منح الرتب · لوحة القرارات |
...
### د) رقّ نفسك إلى مدير أعلى — **مرة واحدة في عمر المشروع**
...
**ولن تحتاج فتح القاعدة بعدها لمنح رتبة.** رتبة المشرف تُمنح وتُنزع من
`/admin/users` داخل الموقع.

أما رتبة `superadmin` فلا واجهة لها إطلاقاً، وهذا ليس نقصاً بل حدّاً أمنياً:
أخطر ثغرة في أي نظام رتب مسارٌ يرقّي به أحدٌ نفسه أو يرقّي من يرقّيه. وإبقاء
أعلى رتبة خارج التطبيق كلياً يقطع السلسلة من أصلها — مهما اخترق مهاجمٌ حساباً
أو ثغرةً في الواجهة، لا يجد طريقاً إلى أعلى. و`set_user_role()` ترفض منحها
صراحةً حتى لو استُدعيت بها مباشرةً.

### هـ) سرّان في GitHub — لبقاء القاعدة حيّة

Supabase توقف المشروع بعد ٧ أيام بلا طلب، فتموت المنصة في الإجازة الفصلية
بالضبط. المهمة المجدولة `.github/workflows/keepalive.yml` تمنع ذلك، وتحتاج:

**GitHub ← المستودع ← Settings ← Secrets and variables ← Actions ← New secret**
...
`anon key` مكشوف بالتصميم (في كود المتصفح أصلاً)، لكنّا لا نكتبه في المستودع:
تاريخ Git يحتفظ بما كُتب فيه للأبد ولو حذفته بعد دقيقة.
**ولا تضع `service_role` هنا إطلاقاً** — الطلب قراءة عامة لا تحتاجه [FIX SEC-03].
