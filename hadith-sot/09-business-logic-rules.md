...
> **تحديث v1.1 — قرارات حسمت غموضاً كُشف في المراجعة:**
>
> | المعرّف | ما كان غامضاً | القرار المعتمد |
> |---|---|---|
> | ALG-C-01 | ALG-001 الطبقة 1: `argmax(my_favorites, likes_count)` بلا كاسر تعادل — نجمتان بنفس عدد الإعجابات ⇒ سلوك غير معرّف | **الأحدث رفعاً** (`created_at DESC`)، اتساقاً مع EC-008 |
> | ALG-C-02 | كاسر التعادل كان مذكوراً في `10-edge-cases.md` فقط لا في الـPseudocode | كاسر التعادل **مطبَّق صراحةً في الطبقات الثلاث** |
> | ALG-C-05 | ALG-003 تناقض بين «5 ثوانٍ متواصلة» و«مؤقّت تراكمي»، ولا قرار حول التقديم السريع | **تراكمي**، و**يُصفَّر عند التقديم السريع (Seek)** منعاً للتلاعب بالعدّاد |
> | ALG-002 | لم يُحسم `>=` أم `>` عند بلوغ العتبة بالضبط | **`>=`** — بلوغ العتبة يكفي |
>
> التنفيذ المرجعي لهذه القرارات: `app/src/lib/algorithms.ts` (مُعلَّم بـ`[FIX ...]`).
...
| ALG-007 | تطبيع النصّ للبحث والتظليل | F010 | **مكانان:** `ar_normalize()` في `09-search.sql` + `fold()` في `src/lib/highlight.ts` |
| ALG-008 | تحديد معدل البلاغات | SEC-09 | مشغّل `enforce_rate_limit()` — `21-rate-limits.sql` |

> ⚠️ **ALG-007 هي الخوارزمية الوحيدة عندنا المنفَّذة مرتين.** القاعدة تطبّع
> لتطابق، والمتصفح يطبّع ليعرف **أين** يظلّل فوق النصّ الأصلي المشكَّل — ولا
> تستطيع القاعدة إعطاءه ذلك (`ts_headline` يُخرج المطبَّع لا الأصل).
>
> التباعد بينهما لا يُنتج انهياراً بل شيئاً أخبث: نتيجةٌ صحيحة بلا تظليل، أو
> تظليل على كلمة لم تطابق. لا خطأ في الطرفية ولا فشل في البناء.
>
> **الحارس:** `npm run test:highlight` (١٩ حالة). أي تعديل في أحد الموضعين
> يُنقل إلى الآخر ويُشغَّل الاختبار قبل الدفع.
